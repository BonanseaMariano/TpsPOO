...
public class CargaDatosController implements Initializable {
    ...
    private void handleAgregar() {
        ...
        if (!camposFaltantes.isEmpty()) {
            ...
            alert.setContentText("Por favor, complete los siguientes campos: " + String.join(", ", camposFaltantes));
            alert.showAndWait();
            return;
        }
        ...
        //El único caso inválido posible para el salario es que solo se coloque un . por eso se asigna un 0
        double salario = 0;
        try {
    ...
    public void validacionDNI() {
        documentoField.textProperty().addListener(new ChangeListener<String>() {
            @Override
    ...
    public void validacionApellido() {
        apellidoField.textProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
    ...
    public void validacionNombre() {
        nombreField.textProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                if (!newValue.matches("[a-zA-Z]*")) {
    ...
    public void validacionSalario() {
        salarioField.textProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                if (!newValue.matches("[0-9.]*")) {
