...
public class EmpleadoAtencion implements Runnable {
  ...
  public EmpleadoAtencion(
      BlockingQueue<Cliente> colaAtencion, AtomicInteger totalClientesAtendidos) {
    ...
    try {
      while (totalClientesAtendidos.get() < Heladeria.NUM_CLIENTES) {
        ...
        if (cliente != null) {
          ...
          Thread.sleep(
              (2 + Heladeria.RANDOM.nextInt(3)) * 1000); // Empleado tarda entre 2 y 4 minutos
          long tiempoTotal = System.currentTimeMillis() - cliente.getTiempoLlegada();
          System.out.println(
              "Cliente " + cliente.getId() + " atendido en " + tiempoTotal / 1000 + " segundos.");
