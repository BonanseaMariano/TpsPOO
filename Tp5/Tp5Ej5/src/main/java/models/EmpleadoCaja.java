...
public class EmpleadoCaja implements Runnable {
  ...
  public EmpleadoCaja(
      BlockingQueue<Cliente> colaCaja,
      BlockingQueue<Cliente> colaAtencion,
      AtomicInteger totalClientesAtendidos) {
    ...
    try {
      while (totalClientesAtendidos.get() < Heladeria.NUM_CLIENTES) {
        ...
        if (cliente != null) {
          ...
          Thread.sleep(
              (1 + Heladeria.RANDOM.nextInt(2)) * 1000); // Cajero tarda entre 1 y 2 minutos
