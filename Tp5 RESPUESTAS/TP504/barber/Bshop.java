...
// Buffer que representa la peluquería
public class Bshop {
    private int nchair;
    ...
    /**
     * Este método simula al peluquero cortando el cabello de un cliente.
     * Espera a que haya un cliente disponible en la cola, luego procesa el corte de cabello.
     * El método está sincronizado para garantizar la seguridad de los hilos al acceder a la cola de clientes.
     */
    public void cutHair() {
        Customer customer;
        ...
        synchronized (listCustomer) {
            ...
            customer = (Customer) ((LinkedList<?>) listCustomer).poll(); // Obtener el primer cliente de la cola
        }
        long duration = 0;
        try {
        ...
        System.out.println("Completado el corte de cabello del cliente: " + customer.getName() + " en " + duration + " segundos.");
    }

    /**
     * Este método agrega un cliente a la cola de la peluquería.
     * Primero verifica si hay una silla disponible para el cliente.
     * Si no hay sillas disponibles, el cliente se va de la tienda.
     * Si hay una silla disponible, el cliente se agrega a la cola.
     * Si el cliente es el único en la cola, notifica al peluquero.
     *
     * @param customer el cliente que se va a agregar a la cola
     */
    public void add(Customer customer) {
        System.out.println("Cliente: " + customer.getName() + " entrando a la tienda a las " + customer.getInTime());
        // Sincronizar el bloqueo
        synchronized (listCustomer) {
