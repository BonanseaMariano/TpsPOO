...
public class Atencion implements Runnable {
    ...
    /**
     * Método que ejecuta el hilo de atención.
     * Recupera clientes de la cola de atención, simula el tiempo de atención y calcula el tiempo total de atención.
     */
    public void run() {
        Cliente cliente;
        long tiempo;
        try {
            while ((cliente = colaAtencion.get()) != null) { // Mientras haya clientes en la cola de atención (el poll no retorne null)
                ...
                tiempo = System.currentTimeMillis() - cliente.getIngreso(); // Calcula el tiempo total de atención
                System.out.println("Cliente: " + cliente.getId() + " Tiempo: " + tiempo);
                cantidad++; // Incrementa la cantidad de clientes atendidos para saber cuántos atendió cada empleado
            }
