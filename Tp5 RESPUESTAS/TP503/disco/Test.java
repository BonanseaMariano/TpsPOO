...
public class Test {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < lec.length; i++) lec[i] = new Read(rwl, i + 1);
    // Crear 2 hilos escritores
    for (int i = 0; i < esc.length; i++) esc[i] = new Write(rwl, i + 1);
    // Crear un pool de hilos
    ExecutorService ejecutorSubprocesos = Executors.newCachedThreadPool();
    // Ejecutar los hilos lectores
    for (int i = 0; i < lec.length; i++) ejecutorSubprocesos.execute(lec[i]);
    // Ejecutar los hilos escritores
    for (int i = 0; i < esc.length; i++) ejecutorSubprocesos.execute(esc[i]);
    // Terminar los hilos
